    ...
    match addr {
        ScAddress::Account(acct) => match &acct.0 {
            PublicKeyTypeEd25519(e) => {
                let strkey = diamnet_strkey::ed25519::PublicKey(e.0);
                write!(f, "{}", strkey)
            }
        },
        ScAddress::Contract(hash) => {
            let strkey = diamnet_strkey::Contract(hash.0);
            write!(f, "{}", strkey)
        }
    }
}
...
fn display_scval(scv: &ScVal, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    match scv {
        ...
        ScVal::ContractInstance(ScContractInstance {
            executable: ContractExecutable::DiamnetAsset,
            ..
        }) => write!(f, "ContractInstance(DiamnetAsset)"),
    }
}
